<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center px-4 relative">
    <div class="w-full max-w-md bg-white shadow-2xl rounded-2xl p-8 sm:p-10 relative z-10 border border-gray-100 text-center">
      
      <div class="absolute top-6 left-6">
        <router-link to="/register" class="p-2 bg-gray-100 rounded-lg hover:bg-gray-200 transition block">
      ...
      <div v-if="authStore.error" class="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg">
        {{ authStore.error }}
      </div>
      ...
      <p class="mt-6 text-sm text-gray-500">
        Didn’t get it? 
        <button @click="handleResend" class="text-[#280559] font-bold hover:underline cursor-pointer">
          Send me a new email
        </button>
      </p>
