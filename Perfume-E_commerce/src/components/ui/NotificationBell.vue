...
<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      <Bell 
        stroke-width="1.5" 
        class="w-6 h-6 xl:w-8 xl:h-8 transition-colors duration-200"
        :class="isOpen ? 'text-[#280559] fill-[#280559]/10' : 'text-black hover:bg-gray-200 rounded-full p-0.5'" 
      />
      
      <div 
        v-if="unreadCount > 0" 
        class="absolute top-0 right-0 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-red-600 border-2 border-white"
      >
    ...
    >
      ...
      <div class="max-h-[28rem] overflow-y-auto">
        <div v-if="sortedNotifications.length === 0" class="py-12 text-center">
          <Bell class="w-12 h-12 text-gray-200 mx-auto mb-3" stroke-width="1" />
        ...
        <div v-for="note in sortedNotifications" :key="note.id">
          ...
          >
            <div v-if="!note.isRead" class="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-[#280559]"></div>
            ...
            <div class="flex items-start space-x-4">
              ...
              >
                <Bell class="w-5 h-5" :class="getTypeConfig(note.type).iconColor" stroke-width="1.5" />
              </div>

              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between mb-1">
                  <span 
                    class="text-[10px] tracking-wider uppercase font-semibold text-gray-400"
                  >
