  ...
  <div class="p-4 md:p-6 space-y-6 min-h-screen bg-gray-50/50">
    <div>
      ...
      <p class="text-sm text-gray-500 mt-1">
        Monitor stock levels, manage brands, and restock products.
      </p>
    </div>
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform transition-all animate-fade-in-up"
      >
        <div class="p-6">
          ...
          <p class="text-sm text-gray-500 mb-6">
            ...
            <span v-if="selectedVariant" class="text-indigo-600 font-bold">
              ({{ selectedVariant.size }})</span
            >
          </p>
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="flex items-center gap-3">
                <button
                  @click="restockAmount = Math.max(1, restockAmount - 1)"
                  class="w-10 h-10 rounded-lg border border-gray-200 flex items-center justify-center hover:bg-gray-50 text-gray-600 transition"
                >
                  -
                </button>
                <input
                  ...
                  type="number"
                  class="flex-1 text-center font-mono font-bold text-lg border-gray-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 bg-gray-50"
                  min="1"
                />
                <button
                  @click="restockAmount++"
            ...
            <div
              class="bg-indigo-50 rounded-lg p-3 flex justify-between items-center text-sm text-indigo-900"
            >
            ...
            <div
              class="bg-green-50 rounded-lg p-3 flex justify-between items-center text-sm text-green-900"
            >
              <span>New Total:</span>
          ...
          <div class="mt-8 flex gap-3">
            <button
              ...
              class="flex-1 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition"
            >
              Cancel
            </button>
            <button
            ...
            >
              <svg v-if="isUpdating" class="animate-spin h-5 w-5 text-white" viewBox="0 0 24 24">
                <circle
                  class="opacity-25"
                  cx="12"
                  cy="12"
    ...
    >
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
        ...
        <div class="flex justify-end gap-2">
          ...
          <button @click="saveNewBrand" class="px-4 py-2 bg-indigo-600 text-white rounded">
            Save
          </button>
        </div>
      </div>
    ...
    >
      <div
        class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[80vh]"
      >
        <div class="p-6 border-b border-gray-100 flex justify-between items-center">
          <div>
            <h3 class="text-lg font-bold text-gray-900">Stock History</h3>
            <p class="text-sm text-gray-500">
              Recent movements for
              <span class="font-medium text-gray-900">{{ selectedProduct?.name }}</span>
            </p>
          </div>
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="(log, i) in historyLogs" :key="i" class="hover:bg-gray-50">
                ...
                <td
                  class="px-6 py-3 text-right font-mono font-bold"
        ...
        <div class="p-4 border-t border-gray-100 bg-gray-50 text-right">
          <button
            ...
            class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 text-gray-700"
          >
            Close
          </button>
        </div>
...
const confirmRestock = async () => {
  ...
  try {
    ...
    if (selectedVariant.value) {
      const vIndex = selectedProduct.value.variants.findIndex(
        (v: any) => v.size === selectedVariant.value.size,
      )
      if (vIndex !== -1) {
        selectedProduct.value.variants[vIndex].stock += restockAmount.value
      }
      selectedProduct.value.stock = selectedProduct.value.variants.reduce(
