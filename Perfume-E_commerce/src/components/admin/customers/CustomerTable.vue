    ...
    <div class="p-4 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row gap-4 justify-between items-center">
      ...
      <div class="flex gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
        <select 
          ...
          @change="$emit('update-filter', 'orderCount', ($event.target as HTMLSelectElement).value)"
        ...
        <select 
          ...
          @change="$emit('update-filter', 'spendingTier', ($event.target as HTMLSelectElement).value)"
          class="px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white text-gray-600 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
        >
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          >
            ...
            <td class="px-6 py-4 text-center font-medium">
              <span v-if="customer.ordersCount && customer.ordersCount > 0" class="px-2 py-0.5 bg-gray-100 rounded-md">
...
defineEmits(['update:searchQuery', 'update-filter', 'view-details', 'export']);
