<template>
  <div
    class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-8 mb-6 transition-all hover:shadow-md"
  >
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <div class="lg:col-span-5 flex flex-col gap-6">
        <div class="flex flex-col gap-4 overflow-y-auto max-h-[220px] pr-2 scrollbar-thin">
          <OrderProduct v-for="(item, index) in order.items" :key="index" :item="item" />
        </div>
        ...
        <div class="mt-12 relative px-2">
          ...
          <div class="relative z-10 flex justify-between w-full">
            <div class="flex flex-col items-center">
              <div
                class="w-4 h-4 rounded-full bg-indigo-900 border-2 border-white shadow-sm ring-1 ring-gray-100"
              ></div>
            ...
            <div class="flex flex-col items-center">
              <div
                class="w-4 h-4 rounded-full border-2 border-white shadow-sm ring-1 ring-gray-100 transition-colors duration-500"
                :class="isStepActive(order.status, 'SHIPPED') ? 'bg-indigo-900' : 'bg-gray-200'"
              ></div>
              <div class="mt-3 text-center">
                <p
                  class="font-medium text-xs text-gray-900"
                  :class="!isStepActive(order.status, 'SHIPPED') && 'text-gray-400'"
                >
                  Shipped
                </p>
            ...
            <div class="flex flex-col items-center">
              ...
              <div class="mt-3 text-center">
                <p
                  class="font-medium text-xs text-gray-900"
                  :class="!isStepActive(order.status, 'DELIVERED') && 'text-gray-400'"
                >
                  Arrived
                </p>
                <p class="text-gray-400 text-[10px]">
                  {{ formatDateShort(order.estimatedArrivalDate) }}
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="lg:col-span-3">
        <div class="bg-gray-50 rounded-xl p-5 border border-gray-200">
          ...
          <div class="space-y-3 text-sm">
            ...
            >
              <span
                >Discount <span v-if="order.promoCode">({{ order.promoCode }})</span></span
              >
            ...
            <div class="pt-3 border-t border-gray-200 flex justify-between items-center mt-2">
              ...
              <span class="font-bold text-xl text-indigo-900">
                ${{
                  formatPrice(order.subtotal + order.shippingCost - (order.discountAmount || 0))
                }}
              </span>
            </div>
          </div>
