<template>
  <div class="min-h-screen bg-gray-50 flex items-center justify-center px-4 relative">
    <div
      class="absolute inset-0 opacity-5 pointer-events-none bg-[url('/Image/HomePage/Main.png')] bg-cover bg-center"
    ></div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-5">
        ...
        <div class="flex items-center justify-between text-sm">
          <label class="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              class="w-4 h-4 text-[#280559] border-gray-300 rounded focus:ring-[#280559]"
            />
            <span class="text-gray-600">Remember me</span>
          </label>

          <router-link to="/forgot-password" class="text-[#280559] font-medium hover:underline">
            Forgot Password?
          </router-link>
        </div>
