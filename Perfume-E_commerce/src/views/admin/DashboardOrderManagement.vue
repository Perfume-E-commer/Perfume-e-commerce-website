  ...
  <div class="p-6 space-y-6 min-h-screen bg-gray-50/50">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage shipments, track revenue, and process payments.</p>
      </div>
      <button 
        @click="loadData" 
    ...
    <div class="space-y-4">
      ...
      <div v-if="filteredOrders.length > pageSize" class="flex items-center justify-between bg-white px-4 py-3 border border-gray-200 rounded-xl shadow-sm">
        <span class="text-sm text-gray-700">
          Showing <span class="font-medium">{{ (currentPage * pageSize) + 1 }}</span> to <span class="font-medium">{{ Math.min((currentPage + 1) * pageSize, filteredOrders.length) }}</span> of <span class="font-medium">{{ filteredOrders.length }}</span> results
        </span>
...
const filteredOrders = computed(() => {
  return rawOrders.value.filter(order => {
    ...
    const nameMatches = (order.user?.firstName + ' ' + order.user?.lastName).toLowerCase().includes(q)
    ...
    const methodMatch = !filters.value.paymentMethod || 
...
const computedStats = computed(() => {
  ...
  return {
    ...
    pendingShipment,
    pendingPayment,
    avgOrderValue: new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(avgOrderValue)
  }
})
