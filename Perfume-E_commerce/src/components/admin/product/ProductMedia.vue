    ...
    <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
      ...
      <div class="flex flex-col md:flex-row gap-6">
        <div class="w-full md:w-1/3">
          <div
            class="aspect-square rounded-lg border-2 border-dashed border-gray-200 flex items-center justify-center bg-gray-50 overflow-hidden relative group"
          >
            <img
              v-if="mainImage"
              :src="mainImage"
        ...
        <div class="w-full md:w-2/3 space-y-4">
          ...
          <div v-if="inputType === 'url'">
            ...
            <input
              :value="mainImage"
              @input="updateVariantsImage(($event.target as HTMLInputElement).value)"
              type="text"
...
const props = defineProps<{
  modelValue: {
    variants: Array<{ imageUrl: string; [key: string]: any }>
    images: string[]
  }
}>()
...
const mainImage = computed(() => {
  if (props.modelValue?.variants?.length > 0) {
    return props.modelValue.variants[0]?.imageUrl || '';
  }
  return '';
});

const updateVariantsImage = (newUrl: string) => {
  if (!props.modelValue?.variants) return;
  
  const updatedVariants = props.modelValue.variants.map(v => ({
    ...v,
    imageUrl: newUrl
  }));

  emit('update:modelValue', { ...props.modelValue, variants: updatedVariants });
};
...
const handleFileUpload = async (event: Event, target: 'main' | 'gallery') => {
  ...
  try {
    ...
    if (target === 'main') {
      updateVariantsImage(uploadedUrl)
    } else {
...
const removeGalleryImage = (index: number) => {
  if (!props.modelValue?.images) return;
