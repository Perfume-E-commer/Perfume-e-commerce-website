  ...
  <div class="p-8 animate-fade-in-up" style="animation-delay: 0.1s;">
    ...
    <div class="transition-all duration-300">
      ...
      <div v-if="!showForm" class="grid grid-cols-1 gap-4">
        ...
        >
          <div v-if="addr.isDefault" class="absolute top-0 right-0 bg-[#280559] text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl rounded-tr-xl z-10 shadow-sm">
            DEFAULT
          </div>

          <div class="flex items-start gap-4">
            
            <div class="shrink-0 w-12 h-12 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-[#280559] shadow-sm mt-1">
            ...
            <div class="flex-grow">
               <div class="flex items-center gap-3 mb-1">
                  ...
                  <span v-if="!addr.isDefault" class="text-[10px] bg-white border border-gray-200 text-gray-400 font-bold px-2 py-0.5 rounded uppercase tracking-wide shadow-sm">{{ addr.type || 'HOME' }}</span>
               </div>
        ...
        <div v-if="addresses.length === 0" class="text-center py-10 px-6 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50">
          <p class="text-gray-500 font-medium text-sm">No addresses saved yet.</p>
          <button @click="showForm = true" class="mt-2 text-[#280559] font-bold text-sm hover:underline">Add one now</button>
        </div>
      </div>
