<template>
  <div class="max-w-7xl mx-auto pb-10">
    ...
    <div v-if="errorMessage" class="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg flex items-center gap-3">
...
const handleSubmit = async () => {
  ...
  // Validation
  if (!formData.value.name || !formData.value.price || !formData.value.brand || !formData.value.category) {
  ...
  if (route.params.id) {
    ...
    try {
      ...
      formData.value = {
        ...
        scentNotes: res.data.scentNotes?.map((note: any) => ({
          type: note.type,
          notes: note.notes || [],
