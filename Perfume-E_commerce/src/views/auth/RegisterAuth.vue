        ...
        <div>
          ...
          <div class="relative">
            <input 
              ...
              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#280559] transition"
            />
            <button type="button" @click="showPass1 = !showPass1" class="absolute right-4 top-3.5 text-gray-400 hover:text-[#280559]">
        ...
        <div>
          ...
          <div class="relative">
            <input 
              ...
              class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#280559] transition"
            />
            <button type="button" @click="showPass2 = !showPass2" class="absolute right-4 top-3.5 text-gray-400 hover:text-[#280559]">
              <span v-if="showPass2">🙈</span><span v-else>👁️</span>
            </button>
          </div>
          <p v-if="passwordMismatch" class="text-red-500 text-xs mt-1 pl-1">Passwords do not match</p>
        </div>
...
async function handleRegister() {
  ...
  const success = await authStore.register(firstName.value, lastName.value, email.value, password.value);
